  ...
  return (
    <div className="w-full p-1 md:p-4">
      <div className="flex justify-between text-xl gap-2">
        ...
        <h1>
          Average Score:{" "}
          <p className={`flex flex-row text-2xl font-bold ${setAllScoreColor()}`}>
            {avgScore}/100
          </p>
        </h1>
      </div>
      ...
      <ul className="grid gap-2 grid-cols-[repeat(auto-fit,minmax(300px,1fr))]">
        {passedTests.map((item) => (
          <li
            key={item.id}
            className={`flex ${setColor(item.correctAnwers, item.questions)} mb-2 p-4 rounded-xl w-[300px] m-auto`}
          >
